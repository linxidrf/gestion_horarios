...
const Horarios = () => {
    ...
    // Función mejorada para verificar configuración activa
    ...
    // UseEffect para cargar configuración inicial y horarios
    useEffect(() => {
        const inicializarDatos = async () => {
            await verificarConfiguracionActiva();
            await obtenerHorario();
        };
        
        inicializarDatos();
    }, [verificarConfiguracionActiva]);

    // UseEffect para recargar horarios cuando cambia la configuración
    useEffect(() => {
    ...
    // Manejo de submit de ajustes mejorado
    const handleAjustesSubmit = async (e) => {
        e.preventDefault();
        
        try {
            // Si ya existe una configuración, preguntar si desea reemplazarla
            if (configuracionActiva) {
            ...
            const response = await guardarConfiguracion({
                semana: semanaInicio,
            ...
            // Asegurar actualización de configuración y horarios
            await verificarConfiguracionActiva();
            await obtenerHorario();
        } catch (error) {
    ...
    // Obtener horario con mejor manejo de errores y logging
    const obtenerHorario = async () => {
        ...
        try {
            ...
            if (!response) {
                console.error("La respuesta de la API es nula o undefined");
                setError("No se recibieron datos del horario");
                return;
            }
            ...
            const errorMensaje = err.response?.data?.error || 
                                  err.message || 
    ...
    // Resto de los métodos existentes (handleGenerarHorario, editarHorario, etc.) 
